...
data_dir = './1_Gen_FT_Data/s1_Raw_Data/nCS'
...
# 遍历目录中的所有文件
for i in range(600):
    file_path = os.path.join(data_dir+path, f"{i}.json")
    # 确保文件存在
        ...
        with open(file_path, 'r') as file:
            ...
            for row in rows:
                ...
                if match is not None:
                    ...
                    texts_CS_key.append({"text": content, "label": 0})
                else:
                    print("None")
...
with open(os.path.join(work_dir, "filter_keywords.json"), 'w', encoding='utf-8') as f:
    json.dump(texts_CS_key, f, ensure_ascii=False, indent=4)
