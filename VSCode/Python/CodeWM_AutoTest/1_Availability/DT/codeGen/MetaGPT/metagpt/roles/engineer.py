...
from metagpt.actions import WriteCodeLocal, WriteCodeReview, WriteTasks
...
class Engineer(Role):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.set_actions([WriteCodeLocal])
        self._watch([WriteTasks, SummarizeCode, WriteCodeLocal, WriteCodeReview, FixBug, WriteCodePlanAndChange])
        self.code_todos = []
        self.summarize_todos = []
        self.next_todo_action = any_to_name(WriteCodeLocal)
    ...
    async def _act(self) -> Message | None:
        ...
        if isinstance(self.rc.todo, WriteCodeLocal):
    ...
    async def _act_write_code(self):
        ...
        return AIMessage(
            content="", cause_by=WriteCodeReview if self.use_code_review else WriteCodeLocal, send_to=MESSAGE_ROUTE_TO_SELF
        )
    ...
    async def _think(self) -> bool:
        ...
        summarize_code_filters = any_to_str_set([WriteCodeLocal, WriteCodeReview])
    ...
    async def _new_code_actions(self):
        ...
        self.code_todos = [
            WriteCodeLocal(i_context=i, repo=self.repo, input_args=self.input_args, context=self.context, llm=self.llm)
