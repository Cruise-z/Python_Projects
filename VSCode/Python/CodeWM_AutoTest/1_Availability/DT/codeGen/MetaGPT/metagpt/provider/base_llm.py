...
class BaseLLM(ABC):
    ...
    async def aask_local(
        ...
        compressed_message = self.compress_messages(message, compress_type=self.config.compress_type)
        rsp0, rsp1 = await self.acompletion_text_local(compressed_message, stream=stream, timeout=self.get_timeout(timeout))
        # rsp = await self.acompletion_text(message, stream=stream, timeout=self.get_timeout(timeout))
        return rsp0, rsp1
    ...
    def get_choice_text_local(self, rsp: dict, choice: int) -> str:
        """Required to provide the first text of choice"""
        message = rsp.get("choices")[choice]["message"]
        if "reasoning_content" in message:
            self.reasoning_content = message["reasoning_content"]
        return message["content"]
